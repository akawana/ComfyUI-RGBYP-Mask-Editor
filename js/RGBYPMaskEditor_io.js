...
function getNodeSuffix(node) {
    try {
        const raw = node.widgets?.find(w => w.name === "rgbyp_json")?.value;
        if (raw) {
            const parsed = JSON.parse(raw);
            if (parsed?.fixed_timestamp) {
                return `${node.id}-${parsed.fixed_timestamp}`;
            }
        }
    } catch (_) {}
    ...
    (async () => {
        ...
        try {
            ...
            if (raw) {
                ...
                if (typeof rawParsed?.fixed_timestamp !== "undefined") {
                    state.rgbypFixedTimestamp = rawParsed.fixed_timestamp;
                }
            }
        } catch (_) {}
        ...
        if (rgbypJson) {
            ...
            const names = buildNamesFromOriginal(rgbypJson.original, getNodeSuffix(node));
            if (names) {
...
export async function saveMask() {
    ...
    const ext = ".png";
    const nodeSuffix = getNodeSuffix(node);

    let names = state.rgbypNames || null;

    if (!names) {
        const tempName = `RGBYPBridge-rgbyp-original-${nodeSuffix}${ext}`;
    ...
    // Preserve fixed_timestamp — never overwrite it
    try {
        const raw = node.widgets?.find(w => w.name === "rgbyp_json")?.value;
        if (raw) {
            const existing = JSON.parse(raw);
            if (existing?.fixed_timestamp) {
                jsonObj.fixed_timestamp = existing.fixed_timestamp;
            }
        }
    } catch (_) {}
