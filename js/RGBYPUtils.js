...
(function () {
    ...
    async function handleLoadMask(node) {
        ...
        // 2) load base image:
        //    - if json.original exists → load that file from clipspace (clean original, not composite)
        //    - otherwise → fall back to node.imgs[0] (first run, no json yet)
        let baseBlob = null;
        let baseW0 = 0;
        let baseH0 = 0;

        const hasOriginalFile = !!(jsonAvailable && originalOutName);

        if (hasOriginalFile) {
            // Load the clean original from clipspace
            try {
                const origFile = await loadFile(originalOutName, "input", "clipspace");
                baseBlob = origFile instanceof Blob ? origFile : await origFile.blob();
                const imgCheck = await blobToImage(baseBlob);
                baseW0 = imgCheck.naturalWidth || imgCheck.width;
                baseH0 = imgCheck.naturalHeight || imgCheck.height;
            } catch (e) {
                rgbypWarn("handleLoadMask: failed to load original from clipspace, falling back to node preview", { originalOutName }, e);
                baseBlob = null;
            }
        }

        if (!baseBlob) {
            // Fallback: use node.imgs[0]
            ...
            baseW0 = baseImg.naturalWidth || baseImg.width || 0;
            baseH0 = baseImg.naturalHeight || baseImg.height || 0;
            ...
            baseBlob = await canvasToBlob(baseCanvas, "image/png", 1.0);
            if (!baseBlob) {
        ...
        // 3) read setting "Downscale to maximum side"
        ...
        if (!hasOriginalFile) {
            // First run — upload original so future runs can load it cleanly
            try {
    ...
    let __rgbyp_image_change_sensitivity = null; // number
    ...
    function initDownscaleCacheFromSettings() {
        try {
            ...
            __rgbyp_image_change_sensitivity = Number(app.extensionManager.setting.get("AK.RGBYP.image_change_sensitivity")) || 20;
        } catch (_) {
            // fallback дефолты
            __rgbyp_ds_enable = false;
            __rgbyp_ds_maxSide = 800;
            __rgbyp_scale_mask_output = false;
            __rgbyp_image_change_sensitivity = 20;
        }
    }

    function setDownscaleFactor(node, fromSettings = false) {
        try {
            ...
            // image_change_sensitivity is only overwritten when the user
            // explicitly changes the setting — not on page load / workflow load.
            if (fromSettings) {
                const wSensitivity = getWidget(node, "image_change_sensitivity");
                if (wSensitivity) {
                    updateWidgetValue(node, "image_change_sensitivity", Number(__rgbyp_image_change_sensitivity) || 20, true);
                }
            }
    ...
    function registerSettings() {
        ...
        S.addSetting({
            ...
            onChange: (newVal, oldVal) => {
                ...
                applyDownscaleToAllBridgeNodes(true);
            },
        });
        S.addSetting({
        ...
        S.addSetting({
            id: "AK.RGBYP.image_change_sensitivity",
            name: "Mask Bridge image change sensitivity (1 to 63):",
            type: "number",
            defaultValue: 20,
            attrs: {
                min: 1,
            ...
            category: ["AK", "RGBYP", "Mask Bridge image change sensitivity (1 to 63)"],
            onChange: (newVal, oldVal) => {
                __rgbyp_image_change_sensitivity = Number(newVal) || 20;
                applyDownscaleToAllBridgeNodes(true);
            },
        });
    ...
    app.registerExtension({
        ...
        beforeRegisterNodeDef(nodeType, nodeData) {
            ...
            nodeType.prototype.onExecuted = function (message) {
                ...
                // Guard: rgbyp_json from server is always an array-wrapped value; extract the string safely
                let v = message?.rgbyp_json;
                if (Array.isArray(v)) {
                    // Only use it if it's a single-element array containing a string — otherwise leave json untouched
                    if (v.length === 1 && (typeof v[0] === "string" || v[0] === null)) {
                        v = v[0] ?? "";
                    } else {
                        // Multiple elements = batch output slipped through; do NOT corrupt the widget
                        console.warn("[RGBYP] onExecuted: unexpected multi-element rgbyp_json array, skipping update", v);
                        return;
                    }
                }
                ...
                // console.log("[RGBYP] returned:", "FALSE");
